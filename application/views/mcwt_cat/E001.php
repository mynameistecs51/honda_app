...
function cancle() {
	if(confirm("ยกเลิกการดำเนินการ หรือไม่ ?"))
	{
		...
		location = base+"/mcwt_cat";
	}
}
function updateMcwt_cat()
  {
    $('#form').on('submit', function (e) {
        ...
        var comment= $('#comment').val(); 
            ...
            {
                type: 'POST',
                url: '<?php echo base_url(); ?>mcwt_cat/saveUpdate/',
...
<?php  foreach ($listMcwt_cat as $detail) { ?>
<input type="hidden"  name="base_url" ID="base_url" value="<?php echo $base_url; ?>">
<input type="hidden"  ID="idx" value="<?php echo $idx; ?>" >
<div class="form_input">
	<p>รหัสประเภทสัญญาบริการ</p>
	<input type="text" class="input" name="mcwt_cat_code" value="<?php echo $detail->mcwt_cat_code;   ?>" readonly>
	<input type="hidden" class="input" name="id_mcwt_cat" value="<?php echo $detail->id_mcwt_cat;   ?>" >
	<p>ชื่อประเภทสัญญาบริการ(ENG)</p>
	<input type="text" class="input" name="name_en" value="<?php echo $detail->name_en; ?>" >
	<p>ชื่อประเภทสัญญาบริการ(TH)</p>
