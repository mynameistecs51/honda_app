...
function cancle() {
	if(confirm("ยกเลิกการดำเนินการ หรือไม่ ?"))
	{
		...
		location = base+"/minv_cat";
	}
}
function updateMinv_cat()
  {
    $('#form').on('submit', function (e) {
        ...
        var comment= $('#comment').val(); 
            ...
            {
                type: 'POST',
                url: '<?php echo base_url(); ?>minv_cat/saveUpdate/',
...
<?php  foreach ($listMinv_cat as $detail) { ?>
<input type="hidden"  name="base_url" ID="base_url" value="<?php echo $base_url; ?>">
<input type="hidden"  ID="idx" value="<?php echo $idx; ?>" >
<div class="form_input">
	<p>รหัสประเภทสินค้า/อะไหล่</p>
	<input type="text" class="input" name="minv_cat_code" value="<?php echo $detail->minv_cat_code;   ?>" readonly>
	<input type="hidden" class="input" name="id_minv_cat" value="<?php echo $detail->id_minv_cat;   ?>" >
	<p>ชื่อประเภทสินค้า/อะไหล่(ENG)</p>
	<input type="text" class="input" name="name_en" id="name_en" value="<?php echo $detail->name_en; ?>" >
	<p>ชื่อประเภทสินค้า/อะไหล่(TH)</p>
