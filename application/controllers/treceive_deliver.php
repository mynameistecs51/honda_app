...
class treceive_deliver extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="treceive_deliver";
		$this->load->model('mdl_treceive_deliver');
...
public function index()
{
	...
	$this->load->view('treceive_deliver/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST;  
    $sqlQuery= $this->mdl_treceive_deliver->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_treceive_deliver->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_treceive_deliver->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMempCat']= $this->mdl_treceive_deliver->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			//$this->data['listMemp']= $this->mdl_treceive_deliver->getMemp($id);
			$this->load->view('treceive_deliver/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_treceive_deliver->addMemp($data);
    endif;
}
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_treceive_deliver->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_treceive_deliver->updateMemp($id,$data);
   ...
   public function get_sel_province()
      {
          ...
          $result=$this->mdl_treceive_deliver->sel_province();
  ...
  public function get_sel_amphur()
  {
      ...
      $result=$this->mdl_treceive_deliver->sel_amphur($province_id);
      foreach ($result as $row)
      {
        $arr[] = $row;
      }
