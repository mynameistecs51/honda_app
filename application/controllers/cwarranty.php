...
class cwarranty extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="cwarranty";
		$this->load->model('mdl_cwarranty'); 
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_cwarranty->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_cwarranty->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_cwarranty->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMempCat']= $this->mdl_cwarranty->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
		//	$this->data['listcwarranty']= $this->mdl_cwarranty->getcwarrantyHdr($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
public function EDIT($id,$idx)
	{
			...
			$this->mainpage($SCREENID); 
			$this->data['idx']=$idx;
		//	$this->data['listcwarranty']= $this->mdl_cwarranty->getcwarranty($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_cwarranty->addMemp($data);
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_cwarranty->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_cwarranty->updateMemp($id,$data);
