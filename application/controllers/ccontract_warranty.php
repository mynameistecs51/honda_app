...
class ccontract_warranty extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="ccontract_warranty";
		$this->load->model('mdl_ccontract_warranty');
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_ccontract_warranty->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_ccontract_warranty->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_ccontract_warranty->getUser($_POST['user']);
	}
}
...
{  
		...
		$this->data["pagename"] =$this->template->getPageName($this->ctl);
		$this->data['listMempCat']= $this->mdl_ccontract_warranty->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage(); 
		//	$this->data['listccontract_warranty']= $this->mdl_ccontract_warranty->getccontract_warrantyHdr($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
public function EDIT($id,$idx)
	{
			...
			$this->mainpage(); 
			$this->data['idx']=$idx;
		//	$this->data['listccontract_warranty']= $this->mdl_ccontract_warranty->getccontract_warranty($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_ccontract_warranty->addMemp($data);
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_ccontract_warranty->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_ccontract_warranty->updateMemp($id,$data);
