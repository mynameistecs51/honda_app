...
class cinstall_std extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="cinstall_std";
		$this->load->model('mdl_cinstall_std');
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_cinstall_std->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_cinstall_std->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_cinstall_std->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMempCat']= $this->mdl_cinstall_std->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
		//	$this->data['listcinstall_std']= $this->mdl_cinstall_std->getcinstall_stdHdr($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
public function EDIT($id,$idx)
	{
			...
			$this->mainpage($SCREENID); 
			$this->data['idx']=$idx;
		//	$this->data['listcinstall_std']= $this->mdl_cinstall_std->getcinstall_std($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_cinstall_std->addMemp($data);
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_cinstall_std->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_cinstall_std->updateMemp($id,$data);
