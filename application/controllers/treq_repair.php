...
class treq_repair extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="treq_repair";
		$this->load->model('mdl_treq_repair');
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_treq_repair->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_treq_repair->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_treq_repair->getUser($_POST['user']);
	}
}
...
{  
		...
		$this->data['listMempCat']= $this->mdl_treq_repair->getMempCat();
...
public function DETAIL($id)
	{
		$SCREENID="D001";
		$this->mainpage(); 
	//	$this->data['listtreq_repair']= $this->mdl_treq_repair->gettreq_repairHdr($id);
		$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
public function EDIT($id,$idx)
	{
		...
		$this->mainpage(); 
		$this->data['idx']=$idx;
	//	$this->data['listtreq_repair']= $this->mdl_treq_repair->gettreq_repair($id);
		$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_treq_repair->addMemp($data);
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_treq_repair->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_treq_repair->updateMemp($id,$data);
