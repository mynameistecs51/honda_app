...
class mwrt_cat extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="mwrt_cat";
		$this->load->model('mdl_mwrt_cat');
...
public function index()
{
	...
	$this->load->view('mwrt_cat/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST;  
    $sqlQuery= $this->mdl_mwrt_cat->getList($requestData);  
...
public function DETAIL($id)
	{ 
			$SCREENID="D001";
			$this->mainpage($SCREENID); 	
			$this->data['listMwrt_cat']= $this->mdl_mwrt_cat->getMwrt($id);
			$this->load->view('mwrt_cat/'.$SCREENID,$this->data);
	}
...
public function saveadd()
{
	if($_POST):
     parse_str($_POST['form'], $post);
		//$code= $this->getCode();  
		$data = array(
			"mwrt_cat_code"		=> $post['mwrt_cat_code'],
		...
		//print_r($data);exit();
			$this->mdl_mwrt_cat->addMwrt_cat($data);
...
public function saveUpdate()
{
	...
	$id=$post['id_mwrt_cat'];
			...
			$this->mdl_mwrt_cat->updateMwrt_cat($id,$data);
