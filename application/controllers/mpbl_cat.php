...
class mpbl_cat extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="mpbl_cat";
		$this->load->model('mdl_mpbl_cat');
...
public function index()
{
	...
	$this->load->view('mpbl_cat/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST;  
    $sqlQuery= $this->mdl_mpbl_cat->getList($requestData);  
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			$this->data['listMpbl_cat']= $this->mdl_mpbl_cat->getMpbl_cat($id);
			$this->load->view('mpbl_cat/'.$SCREENID,$this->data);
	}
...
public function saveadd()
{
	if($_POST):
     parse_str($_POST['form'], $post);
		//$code= $this->getCode();  
		$data = array(
			"mpbl_cat_code"		=> $post['mpbl_cat_code'],
		...
		//print_r($data);exit();
			$this->mdl_mpbl_cat->addMpbl_cat($data);
...
public function saveUpdate()
{
	...
	$id=$post['id_mpbl_cat'];
			...
			$this->mdl_mpbl_cat->updateMpbl_cat($id,$data);
