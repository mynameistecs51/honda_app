...
class minv_cat extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="minv_cat";
		$this->load->model('mdl_minv_cat');
...
public function index()
{
	...
	$this->load->view('minv_cat/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST; 
    $sqlQuery= $this->mdl_minv_cat->getList($requestData);  
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			$this->data['listMinv_cat']= $this->mdl_minv_cat->getMinv_cat($id);
			$this->load->view('minv_cat/'.$SCREENID,$this->data);
	}
...
public function saveadd()
{
	if($_POST):
     parse_str($_POST['form'], $post);
		//$code= $this->getCode();  
		$data = array(
			"minv_cat_code"			=> $post['minv_cat_code'],
		...
		//print_r($data);exit();
			$this->mdl_minv_cat->addMinv_cat($data);
...
public function saveUpdate()
{
	...
	$id=$post['id_minv_cat'];
			...
			$this->mdl_minv_cat->updateMinv_cat($id,$data);
