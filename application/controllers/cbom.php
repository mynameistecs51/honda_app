...
class cbom extends CI_Controller 
{ 
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_cbom->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_cbom->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_cbom->getUser($_POST['user']);
	}
}
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
		//	$this->data['listcbom']= $this->mdl_cbom->getcbomHdr($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
public function EDIT($id,$idx)
	{
			...
			$this->mainpage($SCREENID); 
			$this->data['idx']=$idx;
		//	$this->data['listcbom']= $this->mdl_cbom->getcbom($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_cbom->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_cbom->updateMemp($id,$data);
