...
class treq_deliver extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="treq_deliver";
		$this->load->model('mdl_treq_deliver');
		...
		$this->dtnow = $now->format('d/m/').$Y.$now->format(' H:i');
		$this->datefrom = "01/".$now->format('m/').$Y.$now->format(' H:i');
		$this->dateto = $now->format('d/m/').$Y.$now->format(' H:i');
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_treq_deliver->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_treq_deliver->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_treq_deliver->getUser($_POST['user']);
	}
}
...
{  
		...
		$this->data["dtnow"] =$this->dtnow;
		$this->data["dateto"] =$this->dateto;
		$this->data['listMempCat']= $this->mdl_treq_deliver->getMempCat();
...
public function DETAIL($id)
	{
		$SCREENID="D001";
		$this->mainpage(); 
	//	$this->data['listtreq_deliver']= $this->mdl_treq_deliver->gettreq_deliverHdr($id);
		$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
public function EDIT($id,$idx)
	{
		...
		$this->mainpage(); 
		$this->data['idx']=$idx;
	//	$this->data['listtreq_deliver']= $this->mdl_treq_deliver->gettreq_deliver($id);
		$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_treq_deliver->addMemp($data);
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_treq_deliver->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_treq_deliver->updateMemp($id,$data);
