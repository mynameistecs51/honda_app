...
class treq_borrowing extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="treq_borrowing";
		$this->load->model('mdl_treq_borrowing');
...
public function index()
{
	...
	$this->load->view('treq_borrowing/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST; 
    $sqlQuery= $this->mdl_treq_borrowing->getList($requestData);  
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_mbom->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_mbom->updateMemp($id,$data);
