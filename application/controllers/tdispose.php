...
class tdispose extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="tdispose";
		$this->load->model('mdl_tdispose');
...
public function index()
{
	...
	$this->load->view('tdispose/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST;  
    $sqlQuery= $this->mdl_tdispose->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_tdispose->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_tdispose->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMempCat']= $this->mdl_tdispose->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			//$this->data['listMemp']= $this->mdl_tdispose->getMemp($id);
			$this->load->view('tdispose/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_tdispose->addMemp($data);
    endif;
}
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_tdispose->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_tdispose->updateMemp($id,$data);
