...
class minv_unt extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="minv_unt";
		$this->load->model('mdl_minv_unt');
...
public function index()
{
	...
	$this->load->view('minv_unt/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST; 
    $sqlQuery= $this->mdl_minv_unt->getList($requestData);  
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_minv_unt->getUser($_POST['user']);
	}
}*/
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			$this->data['listMinv_unt']= $this->mdl_minv_unt->getMinv_unt($id);
			$this->load->view('minv_unt/'.$SCREENID,$this->data);
	}
...
public function saveadd()
{
	if($_POST):
     parse_str($_POST['form'], $post);
		//$code= $this->getCode();  
		$data = array(
			"minv_unt_code"		=> $post['minv_unt_code'],
			...
			//print_r($data);exit();
			$this->mdl_minv_unt->addMinv_unt($data);
...
public function saveUpdate()
{
	...
	$id=$post['id_minv_unt'];
			...
			$this->mdl_minv_unt->updateMinv_unt($id,$data);
