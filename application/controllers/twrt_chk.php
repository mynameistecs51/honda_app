...
class twrt_chk extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="twrt_chk";
		$this->load->model('mdl_twrt_chk'); 
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_twrt_chk->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_twrt_chk->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_twrt_chk->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMempCat']= $this->mdl_twrt_chk->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
		//	$this->data['listtwrt_chk']= $this->mdl_twrt_chk->gettwrt_chkHdr($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
