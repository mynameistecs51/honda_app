...
class mdspse_cat extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="mdspse_cat";
		$this->load->model('mdl_mdspse_cat');
...
public function index()
{
	...
	$this->load->view('mdspse_cat/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST; 
    $sqlQuery= $this->mdl_mdspse_cat->getList($requestData);  
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			$this->data['listMdspse_cat']= $this->mdl_mdspse_cat->getMdspse_cat($id);
			$this->load->view('mdspse_cat/'.$SCREENID,$this->data);
	}
...
public function saveadd()
{
	if($_POST):
     parse_str($_POST['form'], $post);
		//$code= $this->getCode();  
		$data = array(
			"mdspse_cat_code"			=> $post['mdspse_cat_code'],
		...
		//print_r($data);exit();
			$this->mdl_mdspse_cat->addMdspse_cat($data);
...
public function saveUpdate()
{
	...
	$id=$post['id_mdspse_cat'];
			...
			$this->mdl_mdspse_cat->updateMdspse_cat($id,$data);
