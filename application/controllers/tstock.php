...
class tstock extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="tstock";
		$this->load->model('mdl_tstock');		
...
public function index()
{
	...
	$this->load->view('tstock/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST;  
    $sqlQuery= $this->mdl_tstock->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_tstock->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_tstock->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMempCat']= $this->mdl_tstock->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			//$this->data['listMemp']= $this->mdl_tstock->getMemp($id);
			$this->load->view('tstock/'.$SCREENID,$this->data);
	}
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_tstock->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_tstock->updateMemp($id,$data);
