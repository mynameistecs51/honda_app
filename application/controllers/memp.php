...
class memp extends CI_Controller 
{ 
...
public function index()
{
	...
	$this->load->view('memp/'.$SCREENID,$this->data);
	
}
public function getList()
{
    $requestData= $_REQUEST;  
    $sqlQuery= $this->mdl_memp->getList($requestData);  
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_memp->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMpst']= $this->mdl_memp->getMpst();
		$this->data['listMdept']= $this->mdl_memp->getMdept();
		...
		$this->data['url_print']=$this->data['base_url'].$this->ctl."/printpdf/";
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
			$this->data['listMemp']= $this->mdl_memp->getMemp($id);
			$this->load->view('memp/'.$SCREENID,$this->data);
	}
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_memp->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_memp->updateMemp($id,$data);
