...
class cwarranty_amount extends CI_Controller 
{ 
	public function __construct()
	{
		parent::__construct();
		$this->ctl="cwarranty_amount";
		$this->load->model('mdl_cwarranty_amount');
...
public function getList()
{
    ...
    $sqlQuery= $this->mdl_cwarranty_amount->getList($requestData);  
...
{
	$mcmp_code='M';
	$lastCode=$this->mdl_cwarranty_amount->getCodeLast($mcmp_code);
	return $lastCode;
}
...
{  
	if ($_POST['user'])
	{ 
		echo $this->mdl_cwarranty_amount->getUser($_POST['user']);
	}
}
...
{ 
		...
		$this->data['listMempCat']= $this->mdl_cwarranty_amount->getMempCat();
...
public function DETAIL($id)
	{
			$SCREENID="D001";
			$this->mainpage($SCREENID); 
		//	$this->data['listcwarranty_amount']= $this->mdl_cwarranty_amount->getcwarranty_amountHdr($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
public function EDIT($id,$idx)
	{
			...
			$this->mainpage($SCREENID); 
			$this->data['idx']=$idx;
		//	$this->data['listcwarranty_amount']= $this->mdl_cwarranty_amount->getcwarranty_amount($id);
			$this->load->view($this->ctl.'/'.$SCREENID,$this->data);
	}
	...
	if($_POST):
     parse_str($_POST['form'], $post);
		...
		$data = array(
			...
			$this->mdl_cwarranty_amount->addMemp($data);
...
public function saveUpdate()
{
    ...
    parse_str($_POST['form'], $post);
		...
		$id=$post['id_memp'];
		if($post['old_pass']!=''){
 			$old_pass=$this->mdl_cwarranty_amount->checkOldPass($id,MD5($post['old_pass']));
		}else{
			$old_pass=1;
		}
		if($old_pass=='1'){
		...
		if($post['pass']!=''){
			...
			$this->mdl_cwarranty_amount->updateMemp($id,$data);
