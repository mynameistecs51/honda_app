<?php
   class mdl_mpbl_cat extends CI_Model
   {
	  ...
	  public function addMpbl_cat($data){
		$this->db->insert('mpbl_cat', $data);
	  }

	  public function updateMpbl_cat($id,$data){
		$this->db->where('id_mpbl_cat', $id);
		$this->db->update('mpbl_cat', $data);
	  }
 ...
 public function getList($requestData){
        ...
        $sql_search.=" AND mpbl_cat_code LIKE '%".$requestData['columns'][0]['search']['value']."%' ";
        } 
...
public function getMpbl_cat($id){
	  $sql = "
			SELECT
				a.id_mpbl_cat,
				a.mpbl_cat_code,
