<?php
   class mdl_ttrf_borrowing extends CI_Model
   {
	  ...
	  public function addTtrfInvHdr($data){
		$this->db->insert('mbom_hdr', $data);
	  }

	  public function addTtrfInvDtl($data){
		$this->db->insert('mbom_dtl', $data);
	  }

	  public function updateTtrfInvHdr($id,$data){
		$this->db->where('id_mbom_hdr', $id);
		$this->db->update('mbom_hdr', $data);
	  }

	  public function updateTtrfInvDtl($id,$data){
		$this->db->where('id_mbom_dtl', $id);
		$this->db->update('mbom_dtl', $data);
	  }
...
public function getCodeLast($mcmp_code){
	$sql = "
				SELECT chuphotic.fn_gen_memp_code('".$mcmp_code."') AS code";
	$query = $this->db->query($sql); 
	if($query->num_rows()>'0'){
		$row = $query->row();
		if($row->code != ""){
