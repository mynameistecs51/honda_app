<?php
   class mdl_mcwt_cat extends CI_Model
   {
	  ...
	  public function addMcwt_cat($data){
		$this->db->insert('mcwt_cat', $data);
	  }

	  public function updateMcwt_cat($id,$data){
		$this->db->where('id_mcwt_cat', $id);
		$this->db->update('mcwt_cat', $data);
	  }
 ...
 public function getList($requestData){
        ...
        $sql_search.=" AND mcwt_cat_code LIKE '%".$requestData['columns'][0]['search']['value']."%' ";
        } 
...
public function getMcwt_cat($id){
	  $sql = "
			SELECT
				a.id_mcwt_cat,
				a.mcwt_cat_code,
