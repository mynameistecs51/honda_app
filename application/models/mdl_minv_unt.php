<?php
   class mdl_minv_unt extends CI_Model
   {
	  ...
	  public function addMinv_unt($data){
		$this->db->insert('minv_unt', $data);
	  }

	  public function updateMinv_unt($id,$data){
		$this->db->where('id_minv_unt', $id);
		$this->db->update('minv_unt', $data);
	  }
 ...
 public function getList($requestData){
        ...
        $sql_search.=" AND a.minv_unt_code LIKE '%".$requestData['columns'][0]['search']['value']."%' ";
        } 
...
public function getMinv_unt($id){
	  $sql = "
			SELECT
				a.id_minv_unt,
				a.minv_unt_code,
